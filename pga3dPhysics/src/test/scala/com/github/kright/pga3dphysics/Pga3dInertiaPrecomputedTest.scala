...
class Pga3dInertiaPrecomputedTest extends AnyFunSuiteLike with ScalaCheckPropertyChecks:
  ...
  test("test precomputed apply method") {
    forAll(Pga3dInertiaGenerators.inertiaMoved, Pga3dGenerators.bivectors, MinSuccessful(100)) { (inertia, b) =>
  ...
  test("test precomputed get acceleration method") {
    forAll(Pga3dInertiaGenerators.inertiaMoved, Pga3dGenerators.bivectors, Pga3dGenerators.bivectors, MinSuccessful(100)) { (inertia, b, f) =>
