...
class ProjectionTest extends AnyFunSuiteLike:
  ...
  test("project point onto line") {
    ...
    val p = Pga3dPointNormalized(px, py, pz).toPoint

    val center = Pga3dPointNormalized(10.0, 20.0, 30.0).toPoint

    val cx = center v x
    val cy = center v y
    val cz = center v z

    assert(p.projectOntoLine(cx) == Pga3dPoint(px, center.y, center.z, w = 1.0))
    assert(p.projectOntoLine(cy) == Pga3dPoint(center.x, py, center.z, w = 1.0))
    assert(p.projectOntoLine(cz) == Pga3dPoint(center.x, center.y, pz, w = 1.0))
  ...
  test("project point onto plane") {
    ...
    assert(p.projectOntoPlane(cxy) == Pga3dPoint(px, py, center.z, w = 1.0))
    assert(p.projectOntoPlane(cxz) == Pga3dPoint(px, center.y, pz, w = 1.0))
    assert(p.projectOntoPlane(cyz) == Pga3dPoint(center.x, py, pz, w = 1.0))
